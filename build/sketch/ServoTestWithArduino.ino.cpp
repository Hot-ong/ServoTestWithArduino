...
#define SIMPLE_MOVING_MODE false    // 그냥 버튼 따라 그 모냥새로 동작
#define SPEED_CONTROL_MODE false    // motor_delay 변수값을 변화시켜 속도 제어 (0일때 최대속도)
#define STEP_MOVING_MODE true       // 하부에 위치한 모터부터 동작 후 상부 모터 동작
...
#line 52 "c:\\Users\\3dore\\Desktop\\HDW\\03_Workspace\\Arduino_Sources\\ServoTestWithArduino\\ServoTestWithArduino.ino"
void setup();
#line 64 "c:\\Users\\3dore\\Desktop\\HDW\\03_Workspace\\Arduino_Sources\\ServoTestWithArduino\\ServoTestWithArduino.ino"
void loop();
#line 119 "c:\\Users\\3dore\\Desktop\\HDW\\03_Workspace\\Arduino_Sources\\ServoTestWithArduino\\ServoTestWithArduino.ino"
void motors_position(int pos);
#line 52 "c:\\Users\\3dore\\Desktop\\HDW\\03_Workspace\\Arduino_Sources\\ServoTestWithArduino\\ServoTestWithArduino.ino"
void setup() {
    pinMode(button_zeroset, INPUT_PULLUP); // 위치 초기화 버튼 핀 할당
    pinMode(button_1st, INPUT_PULLUP);    // 1번 위치 버튼 핀 할당
    pinMode(button_2nd, INPUT_PULLUP);    // 2번 위치 버튼 핀 할당
    pinMode(button_3rd, INPUT_PULLUP);    // 3번 위치 버튼 핀 할당
    pinMode(button_4th, INPUT_PULLUP);    // 4번 위치 버튼 핀 할당
...
void motors_position(int pos)
{
    ...
    if(pres_pos[0] == goal_pos[0])  pos_flag[0];
    if(pres_pos[1] == goal_pos[1])  pos_flag[1];
    if(pres_pos[2] == goal_pos[2])  pos_flag[2];
    

    while (!(pos_flag[0] && pos_flag[1] && pos_flag[2]))
    {
